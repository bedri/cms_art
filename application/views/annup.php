...
<div id="adminContent" class="floatLeft">
	<div class="adminContentItem">
		...
		<div class="adminContentItemList floatLeft">
			<input class="input" style="width: 300px;" type="text" name="name" value="<?php echo $announcementName; ?>">
		</div>
	</div>
	<div class="adminContentItem">
		<div class="adminContentItemList floatLeft bold">
		...
		<div class="adminContentItemList floatLeft">
			<textarea id="description" class="textarea" style="width: 300px; height: 100px;" name="description"><?php echo $announcementDescription; ?></textarea>
		</div>
	</div>
	<div class="adminContentItem">
		<div class="adminContentItemList floatLeft bold">
...
$(document).ready(function() {
	...
	$("#saveButton").click(function() {
		var id= <?php echo $annId; ?>;
		var name = $("input[name=name]").val();
		var description = $("#description").val();
		var coverImage = $("#coverImage").attr('src').replace('/photos/','');
		$.post('/admin/annup/',{"id":id, "name":name, "description":description, "coverImage":coverImage, "action":"update"},function(returned){
			if(returned.success) {
	...
	$("#cancelButton").click(function() {
		...
		var name = $("input[name=name]").val();
		var coverImage = $("#coverImage").attr('src').replace('/photos/','');
		$.post('/admin/annup/',{"id":id, "coverImage":coverImage,"coverImageOrig":coverImageOrig, "action":"cancel"},function(returned){
			if(returned.success) {
				console.log(returned.data);
				window.location='<?echo $baseUrl; ?>admin/annlist';
			}
