...
public function getGoogleLinks($file_type="mp3",$keyword="",$lang="en",$ggl_start_record) {
	...
	$duck_url = "https://duckduckgo.com/?q=%22Index+of%22+AND+%22last+modified%22+AND+%22parent+directory%22+AND+description+AND+size+AND+%28mp3%7Cwma%7Cogg%7Cflac%7Cau%7Cpdf%7Cdoc%7Cmobi%7Clit%7Cdocx%7Ctxt%7Cps%7Crtf%7Cxls%7Cppt%7Cpps%7Cxlsx%7Csql%7Csub%7Csrt%7Cdjvu%7Csit%7Cwebm%7Cogv%7Cmp4%7Cm4v%7Cavi%7Cflv%7Cmkv%7Cdivx%7Cmpeg%7Cmpg%7Cwmv%7Casf%7Cdwg%7Ctorrent%7Capk%7Cjpg%7Cgif%7Cpng%7Ctiff%7Cbmp%7Cpsd%7Ceps%7Cai%29+AND+-php+AND+-html+AND+-asp+AND+-htm+AND+-mp3toss+AND+-wallywashis+AND+-beemp3s+AND+-index-of-mp3+AND+-girlshopes+AND+-theindexof+-plasamusic+-allcandl+-freewarefulldownload+-shexy+-hotbookee";
...
public function getDuckDuckGoLinks($keyword="a") {
	global $curl;

	$duck_links = array();
	$duck_url = "https://duckduckgo.com/?q=$keyword+%22Index+of%22+AND+%22last+modified%22+AND+%22parent+directory%22+AND+description+AND+size+AND+%28mp3%7Cwma%7Cogg%7Cflac%7Cau%7Cpdf%7Cdoc%7Cmobi%7Clit%7Cdocx%7Ctxt%7Cps%7Crtf%7Cxls%7Cppt%7Cpps%7Cxlsx%7Csql%7Csub%7Csrt%7Cdjvu%7Csit%7Cwebm%7Cogv%7Cmp4%7Cm4v%7Cavi%7Cflv%7Cmkv%7Cdivx%7Cmpeg%7Cmpg%7Cwmv%7Casf%7Cdwg%7Ctorrent%7Capk%7Cjpg%7Cgif%7Cpng%7Ctiff%7Cbmp%7Cpsd%7Ceps%7Cai%29+AND+-php+AND+-html+AND+-asp+AND+-htm+AND+-mp3toss+AND+-wallywashis+AND+-beemp3s+AND+-index-of-mp3+AND+-girlshopes+AND+-theindexof+-plasamusic+-allcandl+-freewarefulldownload+-shexy+-hotbookee";

	$duck_content = $curl->get($duck_url);
	preg_match_all("/(href\=\")([^\?\"]*)(\")/i", $duck_content, $duck_matches);
	print_r($duck_matches); exit();

	foreach($duck_matches[2] as $key => $link)
	{
		$base_link = $this->getBaseOpenDirectory($link);
		array_push($duck_links,$base_link);
		echo "\n\033[0;32m[SITE]\033[0;37m $base_link \n";
	}
...
public function getBaseOpenDirectory($url) {
	...
	unset($conditionApache);
	unset($conditionApache2);
	unset($conditionApache3);
	unset($conditionApache4);
	unset($conditionOther);
	unset($conditionOther2);
	unset($conditionOther3);
...
public function getFilesSoundClick($url) {
	...
	foreach($songids_array as $key_fa => $songids_links)
	{
		...
		if(is_array($file_links)) $files = array_unique($file_links[2]);

		foreach($files as $key_fl=>$value_fl)
		{
			$link = urldecode($value_fl);
			$link = addslashes($link);
			$filename_array = explode(".",$link);
			$file_extention = $filename_array[count($filename_array)-1];
			
			$soundclick_que = $db->query("SELECT count(id) FROM music WHERE link='$link' LIMIT 1;");
			...
			{
				echo "\033[36m[FILE EXISTS]\033[37m $link\n\r";
			}
		}
		unset($files_links);
	}
...
public function getFilesMusic($url,$file_types,$keyword="") {
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		if($key_fa && in_array($file_extention,$file_types))
		{
			...
			$music_que = $db->query("SELECT id FROM music WHERE link='$link' LIMIT 1;");
...
public function getFilesMusicHtml($url,$file_types,$keyword="") {
	global $curl;
	global $db;
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		if($key_fa && in_array($file_extention,$file_types))
		{
			...
			/* Checking if file exists */
				...
				if($filesize = $curl->remoteFileExists($files_links))
				{
					$timestamp = time();
					$filesize = (int)$curl->getFileSize($url.$files_links);
...
public function getFilesDocument($url,$file_types,$keyword="") {
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		$link = addslashes($url.$files_links);
		$doc_que = $db->query("SELECT id FROM document WHERE link='$link' LIMIT 1;");
...
public function getFilesDocumentHtml($url,$file_types,$keyword="") {
	global $curl;
	global $db;

	$files = array();

	$site_get_content = $curl->query($url);
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		if(!$record_check && !strstr($link,"'") && !strstr($link,"%27"))
		{
			...
			if($key_fa && in_array($file_extention,$file_types))
			{
				...
				/* Checking if file exists */
				if($filesize = (int)$curl->remoteFileExists($files_links))
				{
					$timestamp = time();

					$document_que = $db->query("SELECT id FROM document WHERE link='$link' LIMIT 1;");
...
public function getFilesVideo($url,$file_types,$keyword="") {
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		$link = addslashes($url.$files_links);
		$video_que = $db->query("SELECT id FROM video WHERE link='$link' LIMIT 1;");
...
public function getFilesArchieve($url,$file_types,$keyword="") {
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		$link = addslashes($url.$files_links);
		$arch_que = $db->query("SELECT id FROM archive WHERE link='$link' LIMIT 1;");
		...
		/* Checking if file exists */
			...
			if($key_fa && in_array($file_extention,$file_types))
			{
				...
				if($filesize = (int)$curl->remoteFileExists($url.$files_links))
				{
					...
					$archive_que = $db->query("SELECT id FROM archive WHERE link='$link' LIMIT 1;");
...
public function getFilesImage($url,$file_types,$keyword="") {
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		if($key_fa && in_array($file_extention,$file_types))
		{
			...
			else echo "\033[36m[FILE EXISTS]\033[37m $link --> Filesize: $file_extention\n\r";
		}
	}
	unset($files);
}
...
public function getFilesTorrent($url,$file_types,$keyword="") {
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		$link = addslashes($url.$files_links);
		$doc_que = $db->query("SELECT id FROM torrent WHERE link='$link' LIMIT 1;");
		$record_check = $db->num_rows($doc_que);
...
public function getFilesAndroid($url,$file_types,$keyword="") {
	...
	foreach($files_array as $key_fa => $files_links)
	{
		...
		$link = addslashes($url.$files_links);
		$doc_que = $db->query("SELECT id FROM android WHERE link='$link' LIMIT 1;");
		$record_check = $db->num_rows($doc_que);
